 ...
 *
 * After each read of trackpad registers, it is necessary to end the
 * communication window by writing the special address 0xEEEE, which causes a
 * NACK from the device. This is a mandatory step according to the IQS5xx
 * protocol.
 *
 ...
 */
static int read_sequence_registers(const struct device *dev, uint16_t reg,
                                   void *val, size_t len) {
  const struct tps43_config *config = dev->config;
  uint8_t addr_buf[2];
 ...
 * @return 0 on success, negative error code on failure
 */
static int tps43_i2c_read_reg16(const struct device *dev, uint16_t reg,
                                uint16_t *val) {
  const struct tps43_config *config = dev->config;
  uint8_t buf[2];
  ...
  // writes register address (reg_buf) and reads 2 bytes of data (into buffer
  // buf)
  ret = i2c_write_read_dt(&config->i2c_bus, reg_buf, sizeof(reg_buf), buf,
                          sizeof(buf));
  if (ret < 0) {
 ...
 * @return 0 on success, negative error code on failure
 */
static __maybe_unused int tps43_i2c_write_reg16(const struct device *dev,
 ...
 * @return 0 on success, negative error code on failure
 */
static int tps43_i2c_read_reg8_w_err(const struct device *dev, uint16_t reg,
                                     uint8_t *val, bool with_err) {
  const struct tps43_config *config = dev->config;
  // forms 2-byte register address: (MSB, LSB)
  uint8_t reg_buf[2] = {reg >> 8, reg & 0xFF};
  int ret;
...
static inline int tps43_i2c_read_reg8(const struct device *dev, uint16_t reg,
                                      uint8_t *val) {
 ...
 * @return 0 on success, negative error code on failure
 */
static int tps43_i2c_write_reg8(const struct device *dev, uint16_t reg,
                                uint8_t val) {
  const struct tps43_config *config = dev->config;
 ...
 */
static void tps43_rdy_callback(const struct device *dev,
                               struct gpio_callback *cb, uint32_t pins) {
  struct tps43_drv_data *drv_data =
      CONTAINER_OF(cb, struct tps43_drv_data, rdy_cb);
...
static void tps43_dump_status(const struct device *dev) {
  ...
  LOG_INF("Charging state: 0x%02X",
          sys_info & TPS43_CHARGING_MODE_MASK); // for debugging charging mode
}
...
static void tps43_force_communication(const struct device *dev) {
  ...
  tps43_i2c_read_reg8_w_err(dev, TPS43_REG_SYSTEM_CONTROL_1, &control_reg,
                            false);
}
 ...
 *
 * Controls the SYSTEM_CONTROL_1 register (0x0432), setting or clearing the
 * SUSPEND bit. In suspend mode, the trackpad enters a low power consumption
 * state and does not process touches until wake-up.
 *
 * @param dev Pointer to trackpad device
 ...
 * @return 0 on success, negative error code on failure
 */
static int tps43_set_suspend_internal(const struct device *dev, bool suspend,
                                      bool lock_held) {
  struct tps43_drv_data *drv_data = dev->data;
  const struct tps43_config *config = dev->config;
  int ret = 0;

  // If power management is disabled, or device is already in the desired state,
  // do nothing
  ...
  // This prevents race condition when RDY fires between suspend attempt and
  // flag setting
...
/** Common swipe handling **/
static void tps43_handle_swipe(const struct device *dev, uint8_t num_fingers,
                               int16_t rel_x, int16_t rel_y) {
  const struct tps43_config *config = dev->config;
  struct tps43_drv_data *drv_data = dev->data;

  if (num_fingers == 1 && !config->single_finger_swipes) {
    return;
  }

  if (num_fingers >= 3 && !config->multi_finger_swipes) {
    return;
  }

  if (rel_x < 0) {
    LOG_INF("%d-finger swipe left - INPUT_BTN_WEST", num_fingers);
    input_report_key(dev, INPUT_BTN_WEST, num_fingers, true, K_FOREVER);
  }
  if (rel_x > 0) {
    LOG_INF("%d-finger swipe right - INPUT_BTN_EAST", num_fingers);
    input_report_key(dev, INPUT_BTN_EAST, num_fingers, true, K_FOREVER);
  }
  if (rel_y < 0) {
    LOG_INF("%d-finger swipe up - INPUT_BTN_NORTH", num_fingers);
    input_report_key(dev, INPUT_BTN_NORTH, num_fingers, true, K_FOREVER);
  }
  if (rel_y > 0) {
    LOG_INF("%d-finger swipe down - INPUT_BTN_SOUTH", num_fingers);
    input_report_key(dev, INPUT_BTN_SOUTH, num_fingers, true, K_FOREVER);
  }
}
...
static void tps43_work_handler(struct k_work *work) {
  struct tps43_drv_data *drv_data =
      CONTAINER_OF(work, struct tps43_drv_data, work);
  ...
  if (is_touching != drv_data->touching) {
    ...
    // input_report_key(dev, INPUT_BTN_TOUCH, is_touching ? 1 : 0, true,
    //                  K_FOREVER);
  }

  if (gestures_events[0] != 0 || gestures_events[1] != 0) {

    LOG_INF("Gestures: Single=0x%02X, Multi=0x%02X", gestures_events[0],
            gestures_events[1]);
    ...
    if (gestures_events[0] & (TPS43_SWIPE_UP | TPS43_SWIPE_DOWN |
                              TPS43_SWIPE_LEFT | TPS43_SWIPE_RIGHT)) {
  ...
  if (rel_x != 0 || rel_y != 0) {
    if (num_fingers >= 3) {
      LOG_INF("Multi-finger movement - checking for swipe");
      tps43_handle_swipe(dev, num_fingers, rel_x, rel_y);
      ...
      if (abs(rel_x) > abs(rel_y)) {
        ...
        LOG_INF("Scrolling %d horizontally", rel_x);
        ...
        LOG_INF("Scrolling %d vertically", rel_y);
      ...
      LOG_INF("Zooming %d", rel_x);
      input_report_rel(dev, INPUT_REL_MISC, rel_x, true, K_FOREVER);
 ...
 * Enables necessary gestures (single tap, press and hold, scroll, two finger
 * tap), configures axis inversion and sets setup complete flag.
 *
 * @param dev Pointer to trackpad device
 * @return 0 on success, negative error code on failure
 */
static int tps43_configure_device(const struct device *dev) {
  ...
  if (config->single_tap || config->press_and_hold || config->scroll ||
      config->two_finger_tap) {
  ...
  if (config->single_tap || config->press_and_hold ||
      config->single_finger_swipes || config->multi_finger_swipes) {
    ...
    single_gestures |= config->single_finger_swipes ? TPS43_SWIPE_UP : 0;
    single_gestures |= config->single_finger_swipes ? TPS43_SWIPE_DOWN : 0;
    single_gestures |= config->single_finger_swipes ? TPS43_SWIPE_LEFT : 0;
    single_gestures |= config->single_finger_swipes ? TPS43_SWIPE_RIGHT : 0;

    ret = tps43_i2c_write_reg8(dev, TPS43_REG_SINGLE_FINGER_GESTURES,
                               single_gestures);
    if (ret != 0) {
  ...
  if (config->two_finger_tap || config->scroll || config->zoom) {
    ...
    ret = tps43_i2c_write_reg8(dev, TPS43_REG_MULTI_FINGER_GESTURES,
                               multi_gestures);
    if (ret != 0) {
  ...
  if (config->filter_dynamic_bottom != -1) {
    ...
    LOG_INF("Dynamic filter bottom set: 0x%02X",
            (uint8_t)config->filter_dynamic_bottom);
  }
  ...
  if (config->filter_dynamic_lower != -1) {
    ...
    LOG_INF("Dynamic filter lower set: 0x%02X",
            (uint8_t)config->filter_dynamic_lower);
  }
  ...
  if (config->filter_dynamic_upper != -1) {
    ...
    LOG_INF("Dynamic filter upper set: 0x%04X",
            (uint16_t)config->filter_dynamic_upper);
  }
  ...
  if (config->swipe_initial_distance != -1) {
    ...
    LOG_INF("Swipe initial distance set: %u px",
            (uint16_t)config->swipe_initial_distance);
  }
  ...
  if (config->swipe_initial_time != -1) {
    ...
    LOG_INF("Swipe initial time set: %u ms",
            (uint16_t)config->swipe_initial_time);
  }
  ...
  if (config->swipe_angle != -1) {
    ...
    LOG_INF("Swipe angle set: %d deg (reg: 0x%02X)", config->swipe_angle,
            reg_val);
  }
  ...
  if (config->swipe_consecutive_distance != -1) {
    ...
    LOG_INF("Swipe consecutive distance set: %u px",
            (uint16_t)config->swipe_consecutive_distance);
  }
  ...
  if (config->swipe_consecutive_time != -1) {
    ...
    LOG_INF("Swipe consecutive time set: %u ms",
            (uint16_t)config->swipe_consecutive_time);
  }
  ...
  if (config->scroll_initial_distance != -1) {
    ...
    LOG_INF("Scroll initial distance set: %u px",
            (uint16_t)config->scroll_initial_distance);
  }
  ...
  if (config->scroll_angle != -1) {
    ...
    LOG_INF("Scroll angle set: %d deg (reg: 0x%02X)", config->scroll_angle,
            reg_val);
  }
  ...
  if (config->zoom_initial_distance != -1) {
    ...
    LOG_INF("Zoom initial distance set: %u px",
            (uint16_t)config->zoom_initial_distance);
  }
  ...
  if (config->zoom_consecutive_distance != -1) {
    ...
    LOG_INF("Zoom consecutive distance set: %u px",
            (uint16_t)config->zoom_consecutive_distance);
  }
  ...
  if (config->report_rate_active != -1) {
    ...
    LOG_INF("Report rate active set: %u ms",
            (uint16_t)config->report_rate_active);
  }
  ...
  if (config->report_rate_idle_touch != -1) {
    ...
    LOG_INF("Report rate idle touch set: %u ms",
            (uint16_t)config->report_rate_idle_touch);
  }
  ...
  if (config->timeout_idle_touch != -1) {
    ...
    LOG_INF("Timeout idle touch set: %u s",
            (uint8_t)config->timeout_idle_touch);
  }
  ...
  ret = tps43_i2c_write_reg8(dev, TPS43_REG_SYSTEM_CONFIG_0,
                             TPS43_SETUP_COMPLETE | TPS43_WDT_ENABLE |
                                 TPS43_REATI);
  if (ret != 0) {
...
static int check_reset_and_reconfigure(const struct device *dev) {
  ...
  if (sys_info & TPS43_SHOW_RESET) {
    ...
    ret = tps43_i2c_write_reg8(dev, TPS43_REG_SYSTEM_CONTROL_0,
...
static void tps43_dump_registers(const struct device *dev) {
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("SYSTEM_CONTROL_0 (0x%04X): 0x%02X", TPS43_REG_SYSTEM_CONTROL_0,
            reg8);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("SYSTEM_CONTROL_1 (0x%04X): 0x%02X", TPS43_REG_SYSTEM_CONTROL_1,
            reg8);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("SYSTEM_CONFIG_0 (0x%04X): 0x%02X", TPS43_REG_SYSTEM_CONFIG_0,
            reg8);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("SYSTEM_CONFIG_1 (0x%04X): 0x%02X", TPS43_REG_SYSTEM_CONFIG_1,
            reg8);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("REF_DRIFT_LIMIT (0x%04X): 0x%02X", TPS43_REG_REF_DRIFT_LIMIT,
            reg8);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("REATI_LOWER_LIMIT (0x%04X): 0x%02X", TPS43_REG_REATI_LOWER_LIMIT,
            reg8);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("REATI_UPPER_LIMIT (0x%04X): 0x%02X", TPS43_REG_REATI_UPPER_LIMIT,
            reg8);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("MAX_COUNT_LIMIT (0x%04X): 0x%04X", TPS43_REG_MAX_COUNT_LIMIT,
            reg16);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("REPORT_RATE_ACTIVE (0x%04X): 0x%04X", TPS43_REG_REPORT_RATE_ACTIVE,
            reg16);
  }

  read_ret =
      tps43_i2c_read_reg16(dev, TPS43_REG_REPORT_RATE_IDLE_TOUCH, &reg16);
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("REPORT_RATE_IDLE_TOUCH (0x%04X): 0x%04X",
            TPS43_REG_REPORT_RATE_IDLE_TOUCH, reg16);
  }

  read_ret = tps43_i2c_read_reg16(dev, TPS43_REG_REPORT_RATE_IDLE, &reg16);
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("REPORT_RATE_IDLE (0x%04X): 0x%04X", TPS43_REG_REPORT_RATE_IDLE,
            reg16);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("REPORT_RATE_LP1 (0x%04X): 0x%04X", TPS43_REG_REPORT_RATE_LP1,
            reg16);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("REPORT_RATE_LP2 (0x%04X): 0x%04X", TPS43_REG_REPORT_RATE_LP2,
            reg16);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("TIMEOUT_IDLE_TOUCH (0x%04X): 0x%02X", TPS43_REG_TIMEOUT_IDLE_TOUCH,
            reg8);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("REF_UPDATE_TIME (0x%04X): 0x%02X", TPS43_REG_REF_UPDATE_TIME,
            reg8);
  }
  ...
  read_ret =
      tps43_i2c_read_reg8(dev, TPS43_REG_XY_DYNAMIC_FILTER_BOTTOM, &reg8);
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("XY_DYNAMIC_FILTER_BOTTOM (0x%04X): 0x%02X",
            TPS43_REG_XY_DYNAMIC_FILTER_BOTTOM, reg8);
  }

  read_ret = tps43_i2c_read_reg8(dev, TPS43_REG_XY_DYNAMIC_FILTER_LOWER, &reg8);
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("XY_DYNAMIC_FILTER_LOWER (0x%04X): 0x%02X",
            TPS43_REG_XY_DYNAMIC_FILTER_LOWER, reg8);
  }

  read_ret =
      tps43_i2c_read_reg16(dev, TPS43_REG_XY_DYNAMIC_FILTER_UPPER, &reg16);
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("XY_DYNAMIC_FILTER_UPPER (0x%04X): 0x%04X",
            TPS43_REG_XY_DYNAMIC_FILTER_UPPER, reg16);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("SWIPE_INITIAL_TIME (0x%04X): %u ms", TPS43_REG_SWIPE_INITIAL_TIME,
            reg16);
  }

  read_ret =
      tps43_i2c_read_reg16(dev, TPS43_REG_SWIPE_INITIAL_DISTANCE, &reg16);
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("SWIPE_INITIAL_DISTANCE (0x%04X): %u px",
            TPS43_REG_SWIPE_INITIAL_DISTANCE, reg16);
  }

  read_ret =
      tps43_i2c_read_reg16(dev, TPS43_REG_SWIPE_CONSECUTIVE_TIME, &reg16);
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("SWIPE_CONSECUTIVE_TIME (0x%04X): %u ms",
            TPS43_REG_SWIPE_CONSECUTIVE_TIME, reg16);
  }

  read_ret =
      tps43_i2c_read_reg16(dev, TPS43_REG_SWIPE_CONSECUTIVE_DISTANCE, &reg16);
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("SWIPE_CONSECUTIVE_DISTANCE (0x%04X): %u px",
            TPS43_REG_SWIPE_CONSECUTIVE_DISTANCE, reg16);
  }
  ...
  read_ret =
      tps43_i2c_read_reg16(dev, TPS43_REG_SCROLL_INITIAL_DISTANCE, &reg16);
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("SCROLL_INITIAL_DISTANCE (0x%04X): %u px",
            TPS43_REG_SCROLL_INITIAL_DISTANCE, reg16);
  }
  ...
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("ZOOM_INITIAL_DISTANCE (0x%04X): %u px",
            TPS43_REG_ZOOM_INITIAL_DISTANCE, reg16);
  }

  read_ret =
      tps43_i2c_read_reg16(dev, TPS43_REG_ZOOM_CONSECUTIVE_DISTANCE, &reg16);
  ret |= read_ret;
  if (read_ret == 0) {
    LOG_INF("ZOOM_CONSECUTIVE_DISTANCE (0x%04X): %u px",
            TPS43_REG_ZOOM_CONSECUTIVE_DISTANCE, reg16);
  }
...
#define TPS43_INIT(inst)                                                       \
  ...
  static const struct tps43_config tps43_##inst##_config = {                   \
      ...
      .single_finger_swipes = DT_INST_PROP(inst, single_finger_swipes),        \
      .multi_finger_swipes = DT_INST_PROP(inst, multi_finger_swipes),          \
      ...
      .enable_power_management =                                               \
          DT_INST_PROP_OR(inst, enable_power_management, true),                \
      .idle_sleep = DT_INST_PROP_OR(inst, idle_sleep, false),                  \
      .filter_settings = DT_INST_PROP_OR(inst, filter_settings, 0x0F),         \
      .filter_dynamic_bottom =                                                 \
          DT_INST_PROP_OR(inst, filter_dynamic_bottom, -1),                    \
      ...
      .swipe_initial_distance =                                                \
          DT_INST_PROP_OR(inst, swipe_initial_distance, -1),                   \
      .swipe_initial_time = DT_INST_PROP_OR(inst, swipe_initial_time, -1),     \
      .swipe_angle = DT_INST_PROP_OR(inst, swipe_angle, -1),                   \
      .swipe_consecutive_distance =                                            \
          DT_INST_PROP_OR(inst, swipe_consecutive_distance, -1),               \
      .swipe_consecutive_time =                                                \
          DT_INST_PROP_OR(inst, swipe_consecutive_time, -1),                   \
      .scroll_initial_distance =                                               \
          DT_INST_PROP_OR(inst, scroll_initial_distance, -1),                  \
      .scroll_angle = DT_INST_PROP_OR(inst, scroll_angle, -1),                 \
      .zoom_initial_distance =                                                 \
          DT_INST_PROP_OR(inst, zoom_initial_distance, -1),                    \
      .zoom_consecutive_distance =                                             \
          DT_INST_PROP_OR(inst, zoom_consecutive_distance, -1),                \
      ...
      .report_rate_idle_touch =                                                \
          DT_INST_PROP_OR(inst, report_rate_idle_touch, -1),                   \
      ...
      .hold_time = DT_INST_PROP_OR(inst, hold_time, -1),                       \
                                                                               ...
                                                                               \
  DEVICE_DT_INST_DEFINE(inst, tps43_init, NULL, &tps43_##inst##_drvdata,       \
                        &tps43_##inst##_config, POST_KERNEL,                   \
                        CONFIG_INPUT_INIT_PRIORITY, NULL);                     \
  BUILD_ASSERT(DT_INST_REG_ADDR(inst) == TPS43_I2C_ADDR,                       \
 ...
 * to put trackpad into sleep mode when keyboard transitions to idle/sleep
 * state.
 *
 * @param dev Pointer to trackpad device
